 ...
 * 它 的属性指定的是Web资源的位置，默认为src/main/webapp，本例修改为src/main/resources。
 */
@WebAppConfiguration("src/main/resources") 
public class TestControllerIntegrationTests {
	private MockMvc mockMvc;// 2 MockMvc模拟Mvc对象，通过MockMvcBuilders.webAppContextSetup(this.wac).build()初始化。
	...
	public void testNormalController() throws Exception {
		mockMvc.perform(get("/normal"))// 8模拟向/normal进行get请求
				...
				.andExpect(forwardedUrl("/WEB-INF/classes/views/page.jsp"))// 11预期页面转向的真正路径为此
				.andExpect(model().attribute("msg", demoService.saySomething()));// 12预期model里的值是hello
	...
	public void testRestController() throws Exception {
		mockMvc.perform(get("/testRest"))// 13模拟向/testRest进行get请求
				...
				.andExpect(content().string(demoService.saySomething()));// 15预期返回值的内容为hello
	}
}
